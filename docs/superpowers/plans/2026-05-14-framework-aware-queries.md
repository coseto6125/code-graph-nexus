...
**Goal:** 在 gnx-rs 加入框架感知 edges（FastAPI Depends、Axum Router::route 等），讓 web 後端依賴圖召回率從 ~50% 提升至 ~90%，同時用 `Edge.confidence` 限制誤判風險。
...
**Create:**
- `crates/gnx-analyzer/src/python/frameworks.scm` — Python 框架 query（FastAPI / Flask 子集）
- `crates/gnx-analyzer/src/rust/frameworks.scm` — Rust 框架 query（Axum / Actix）
- `crates/gnx-analyzer/src/typescript/frameworks.scm` — TS 框架 query（Express / NestJS）
- `crates/gnx-cli/tests/fixtures/fastapi_depends.py` — fixture
- `crates/gnx-cli/tests/fixtures/axum_router.rs` — fixture
- `crates/gnx-cli/tests/fixtures/express_app.ts` — fixture
- `crates/gnx-cli/tests/framework_aware.rs` — 整合測試

**Modify:**
- `crates/gnx-core/src/analyzer/types.rs` — 加 `RawFrameworkRef` + `LocalGraph.framework_refs`
- `crates/gnx-analyzer/src/python/parser.rs` — 合併 .scm + 處理 framework captures
- `crates/gnx-analyzer/src/rust/parser.rs` — 同上
- `crates/gnx-analyzer/src/typescript/parser.rs` — 同上
- `crates/gnx-analyzer/src/resolution/builder.rs` — Pass 2 解析 `framework_refs` → `Edge`
- `crates/gnx-cli/src/commands/detect_changes.rs` — 加 `--high-trust-only` flag
...
**Files:**
- Modify: `crates/gnx-core/src/analyzer/types.rs`
- Modify: `crates/gnx-analyzer/src/resolution/builder.rs`
- Test: `crates/gnx-analyzer/src/resolution/builder.rs` (inline `#[cfg(test)]`)
...
- [ ] **Step 2: Run** `cargo test -p gnx-analyzer framework_ref_produces_edge` — expect compile FAIL（`RawFrameworkRef` 不存在）。

- [ ] **Step 3: Add `RawFrameworkRef` type** to `crates/gnx-core/src/analyzer/types.rs`：
...
- [ ] **Step 4: Update all `LocalGraph` construction sites** to include `framework_refs: Vec::new()`. Use grep `LocalGraph {` across `crates/gnx-analyzer/src/*/parser.rs`. Should be ~20 files; default empty.
...
- [ ] **Step 6: Run** test — expect PASS。Run 整個 `cargo test -p gnx-analyzer` 確認沒退化。

- [ ] **Step 7: Commit**

```bash
git add crates/gnx-core/src/analyzer/types.rs crates/gnx-analyzer/src/resolution/builder.rs crates/gnx-analyzer/src/*/parser.rs
...
**Files:**
- Create: `crates/gnx-analyzer/src/python/frameworks.scm`
- Modify: `crates/gnx-analyzer/src/python/parser.rs`
- Create: `crates/gnx-cli/tests/fixtures/fastapi_depends.py`
- Create or extend test in `crates/gnx-cli/tests/framework_aware.rs`
...
fn fastapi_depends_creates_low_confidence_reference() {
    ...
    let provider = gnx_analyzer::python::PythonProvider::new().unwrap();
...
```bash
git add crates/gnx-analyzer/src/python/frameworks.scm crates/gnx-analyzer/src/python/parser.rs crates/gnx-cli/tests/fixtures/fastapi_depends.py crates/gnx-cli/tests/framework_aware.rs
...
**Files:**
- Create: `crates/gnx-analyzer/src/rust/frameworks.scm`
- Modify: `crates/gnx-analyzer/src/rust/parser.rs`
- Create: `crates/gnx-cli/tests/fixtures/axum_router.rs.txt` (avoid Cargo picking it up — `.txt` extension; `include_str!` 處用相對路徑)
- Extend: `crates/gnx-cli/tests/framework_aware.rs`
...
**Files:**
- Create: `crates/gnx-analyzer/src/typescript/frameworks.scm`
- Modify: `crates/gnx-analyzer/src/typescript/parser.rs`
- Create: `crates/gnx-cli/tests/fixtures/express_app.ts`
- Extend: `crates/gnx-cli/tests/framework_aware.rs`
...
**Files:**
- Modify: `crates/gnx-cli/src/commands/detect_changes.rs`
- Modify: `crates/gnx-cli/src/commands/impact.rs`
- Extend: `crates/gnx-cli/tests/framework_aware.rs`
