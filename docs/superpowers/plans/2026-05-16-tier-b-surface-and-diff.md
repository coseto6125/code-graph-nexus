# Tier B CLI Surface + `gnx diff` Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Move `mcp` / `verify_resolver` to `admin` namespace, un-hide `shape_check` at top-level with `--route`, and introduce `gnx diff --section <bindings|routes|contracts|all> --baseline <ref>` as the generalized graph-level cross-commit diff command.

**Architecture:** Visibility / namespace refactor for three existing hidden commands; new `commands/diff/` module containing baseline-ref resolution, git stash/checkout RAII helper, three section diff implementations, and three output formatters. Uses existing `gnx admin index --dump-resolver` for bindings dump; loads two `graph.bin` files for routes / contracts comparison.

**Tech Stack:** Rust 2021, clap 4.5 derive, serde_json, rkyv-archived graph, git CLI (via `std::process::Command`), `gh` CLI (for PR/<n> resolution), existing `graph_nexus_core::graph` + `graph_nexus_core::analyzer::types` APIs.
...
| File | Change |
|---|---|
| `crates/graph-nexus-cli/src/main.rs` | Remove `Commands::Mcp` / `Commands::VerifyResolver` variants + their dispatch; remove `#[command(hide = true)]` on `Commands::ShapeCheck`; add new `Commands::Diff(commands::diff::DiffArgs)` variant + dispatch |
| `crates/graph-nexus-cli/src/commands/mod.rs` | Add `pub mod diff;` |
| `crates/graph-nexus-cli/src/commands/admin/mod.rs` | Add `Mcp(McpArgs)` + `VerifyResolver(VerifyResolverArgs)` variants to `AdminCommands` enum + dispatch in `run()` |
| `crates/graph-nexus-cli/src/commands/shape_check.rs` | Add `route: Option<String>` to `ShapeCheckArgs`; filter Fetches edges by route path in `run()` |
| `crates/graph-nexus-cli/src/commands/mcp.rs` | Add `format: Option<OutputFormat>` to `McpAction::Tools`; emit tools list via configured formatter |
| `~/.claude/skills/gnx/SKILL.md` | Append rows for `shape_check --route` and `diff --section --baseline`; note `mcp` / `verify-resolver` are admin-namespaced |

### Files to create

| File | Responsibility |
|---|---|
| `crates/graph-nexus-cli/src/commands/diff/mod.rs` | `DiffArgs` struct, dispatch entry, section composition |
| `crates/graph-nexus-cli/src/commands/diff/baseline.rs` | Resolve `--baseline <ref>` to commit SHA (branch / tag / SHA / HEAD~N / `PR/<n>`) |
| `crates/graph-nexus-cli/src/commands/diff/git_guard.rs` | RAII helper that stashes dirty tree + checkouts SHA + restores on drop |
| `crates/graph-nexus-cli/src/commands/diff/bindings.rs` | `bindings` section: compare two resolver-dump JSONLs |
| `crates/graph-nexus-cli/src/commands/diff/routes.rs` | `routes` section: compare Route nodes between two graphs |
| `crates/graph-nexus-cli/src/commands/diff/contracts.rs` | `contracts` section: compare cross-repo contracts |
| `crates/graph-nexus-cli/src/commands/diff/output.rs` | Format text / json / toon |
| `crates/graph-nexus-cli/tests/admin_mcp_test.rs` | Verify `admin mcp tools --format toon` works; not visible at top-level |
| `crates/graph-nexus-cli/tests/admin_verify_resolver_test.rs` | Verify `admin verify-resolver` dispatch |
| `crates/graph-nexus-cli/tests/shape_check_route_filter.rs` | Verify `--route` filter |
| `crates/graph-nexus-cli/tests/diff_baseline_resolve_test.rs` | Verify baseline ref resolution variants |
| `crates/graph-nexus-cli/tests/diff_bindings_test.rs` | Verify bindings section diff |
| `crates/graph-nexus-cli/tests/diff_routes_test.rs` | Verify routes section diff |
| `crates/graph-nexus-cli/tests/diff_contracts_test.rs` | Verify contracts section diff |
| `crates/graph-nexus-cli/tests/diff_section_all_test.rs` | Verify `--section all` equivalence to multi-select |
| `crates/graph-nexus-cli/tests/cli_help_surface_test.rs` | Snapshot top-level / admin `--help` |
...
**Files:**
- Modify: `crates/graph-nexus-cli/src/commands/admin/mod.rs:14-39`
- Modify: `crates/graph-nexus-cli/src/main.rs:80-83` (remove top-level variant), `:125-133` (remove early dispatch), `:148`, `:154-155`, `:191-192` (remove from no-graph fall-through arms)
- Test: `crates/graph-nexus-cli/tests/admin_mcp_test.rs` (create)

- [ ] **Step 1: Write the failing test**

Create `crates/graph-nexus-cli/tests/admin_mcp_test.rs`:
...
Run: `cargo test -p graph-nexus --test admin_mcp_test`
...
Edit `crates/graph-nexus-cli/src/commands/admin/mod.rs`:
...
Edit `crates/graph-nexus-cli/src/main.rs`:
...
```bash
git add crates/graph-nexus-cli/src/commands/admin/mod.rs \
        crates/graph-nexus-cli/src/main.rs \
        crates/graph-nexus-cli/tests/admin_mcp_test.rs
...
**Files:**
- Modify: `crates/graph-nexus-cli/src/commands/admin/mod.rs` (add variant + dispatch)
- Modify: `crates/graph-nexus-cli/src/main.rs` (remove top-level)
- Test: `crates/graph-nexus-cli/tests/admin_verify_resolver_test.rs` (create)

- [ ] **Step 1: Write the failing test**

Create `crates/graph-nexus-cli/tests/admin_verify_resolver_test.rs`:
...
Run: `cargo test -p graph-nexus --test admin_verify_resolver_test`
...
Edit `crates/graph-nexus-cli/src/commands/admin/mod.rs` — add to `AdminCommands` enum (after `Mcp`):
...
git add crates/graph-nexus-cli/src/commands/admin/mod.rs \
        crates/graph-nexus-cli/src/main.rs \
        crates/graph-nexus-cli/tests/admin_verify_resolver_test.rs
...
**Files:**
- Modify: `crates/graph-nexus-cli/src/main.rs` (~line 77-79: remove `#[command(hide = true)]`)
- Test: `crates/graph-nexus-cli/tests/cli_help_surface_test.rs` (create)

- [ ] **Step 1: Write the failing test**

Create `crates/graph-nexus-cli/tests/cli_help_surface_test.rs`:
...
Run: `cargo test -p graph-nexus --test cli_help_surface_test`
...
Edit `crates/graph-nexus-cli/src/main.rs` — find `Commands::ShapeCheck` variant (around line 77-79). Remove the `#[command(hide = true)]` attribute. Update the doc comment to be user-facing:
...
**Files:**
- Modify: `crates/graph-nexus-cli/src/commands/shape_check.rs`
- Test: `crates/graph-nexus-cli/tests/shape_check_route_filter.rs` (create)

- [ ] **Step 1: Write the failing test**

Create `crates/graph-nexus-cli/tests/shape_check_route_filter.rs`:
...
fn shape_check_help_lists_route_arg() {
    let output = Command::new(gnx_bin())
        ...
        .output()
        .expect("run gnx shape_check --help");
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
...
fn shape_check_route_no_match_emits_helpful_message() {
    ...
    let output = Command::new(gnx_bin())
        ...
        .output()
        .expect("run gnx shape_check --route");
...
Run: `cargo test -p graph-nexus --test shape_check_route_filter`
Expected: `shape_check_help_lists_route_arg` fails — `--route` not recognized.

- [ ] **Step 3: Add `--route` to `ShapeCheckArgs`**

Edit `crates/graph-nexus-cli/src/commands/shape_check.rs`:
...
```bash
git add crates/graph-nexus-cli/src/commands/shape_check.rs \
        crates/graph-nexus-cli/tests/shape_check_route_filter.rs
...
**Files:**
- Modify: `crates/graph-nexus-cli/src/commands/mcp.rs`
- Test: `crates/graph-nexus-cli/tests/admin_mcp_test.rs` (extend)
...
Run: `cargo test -p graph-nexus --test admin_mcp_test -- admin_mcp_tools_json_format`
Expected: fails because `--format` is not yet recognized.

- [ ] **Step 3: Update `McpAction::Tools` to accept `--format`**

Edit `crates/graph-nexus-cli/src/commands/mcp.rs`:
            ...
            match format.as_str() {
                ...
                "toon" => {
                    ...
                    let json = serde_json::to_value(&tools)
                        .map_err(|e| GnxError::Output(format!("toon: {e}")))?;
                    println!("{}", etoon::to_string(&json)
                        .map_err(|e| GnxError::Output(format!("toon emit: {e}")))?);
                }
                _ => {
...
```bash
git add crates/graph-nexus-cli/src/commands/mcp.rs \
        crates/graph-nexus-cli/tests/admin_mcp_test.rs
...
### Task 6: Skeleton `gnx diff` command (DiffArgs + dispatch wired with stub)

**Files:**
- Create: `crates/graph-nexus-cli/src/commands/diff/mod.rs`
- Modify: `crates/graph-nexus-cli/src/commands/mod.rs` (add `pub mod diff;`)
- Modify: `crates/graph-nexus-cli/src/main.rs` (add `Commands::Diff(diff::DiffArgs)` variant + dispatch)
- Test: `crates/graph-nexus-cli/tests/diff_baseline_resolve_test.rs` (create with stub test)

- [ ] **Step 1: Write the failing test**

Create `crates/graph-nexus-cli/tests/diff_baseline_resolve_test.rs`:
...
Run: `cargo test -p graph-nexus --test diff_baseline_resolve_test`
...
pub fn run(args: DiffArgs) -> Result<(), GnxError> {
    ...
    let _ = args;
    Err(GnxError::Output(
        "gnx diff not yet implemented; tracking issue: tier-b plan task 6".into(),
    ))
}
```
...
Edit `crates/graph-nexus-cli/src/main.rs` — add variant after `Contracts`:
...
```bash
git add crates/graph-nexus-cli/src/commands/diff/mod.rs \
        crates/graph-nexus-cli/src/commands/mod.rs \
        crates/graph-nexus-cli/src/main.rs \
        crates/graph-nexus-cli/tests/diff_baseline_resolve_test.rs
...
**Files:**
- Create: `crates/graph-nexus-cli/src/commands/diff/baseline.rs`
- Modify: `crates/graph-nexus-cli/tests/diff_baseline_resolve_test.rs` (extend)

- [ ] **Step 1: Append failing tests**

Append to `crates/graph-nexus-cli/tests/diff_baseline_resolve_test.rs`:
...
fn diff_baseline_pr_form_calls_gh() {
    ...
    // Use a known non-existent PR; expectation: clean error from gh, surfaced by gnx.
    let output = Command::new(env!("CARGO_BIN_EXE_gnx"))
...
pub fn resolve(ref_str: &str, repo_dir: &std::path::Path) -> Result<String, GnxError> {
...
fn resolve_via_git(ref_str: &str, repo_dir: &std::path::Path) -> Result<String, GnxError> {
...
fn resolve_pr(pr_num: &str, repo_dir: &std::path::Path) -> Result<String, GnxError> {
...
Edit `crates/graph-nexus-cli/src/commands/diff/mod.rs` — replace stub:
...
```bash
git add crates/graph-nexus-cli/src/commands/diff/baseline.rs \
        crates/graph-nexus-cli/src/commands/diff/mod.rs \
        crates/graph-nexus-cli/tests/diff_baseline_resolve_test.rs
...
**Files:**
- Create: `crates/graph-nexus-cli/src/commands/diff/git_guard.rs`
...
fn git_guard_restores_branch_on_drop() {
    ...
    // Use the gnx binary to run a stub that exercises guard then restores.
...
Run: `cargo test -p graph-nexus --test diff_baseline_resolve_test -- git_guard_restores_branch_on_drop`
...
impl GitGuard {
    ...
    pub fn enter(repo_dir: &std::path::Path, target_sha: &str) -> Result<Self, GnxError> {
...
fn current_head_ref(repo_dir: &std::path::Path) -> Result<String, GnxError> {
...
fn stash_if_dirty(repo_dir: &std::path::Path) -> Result<bool, GnxError> {
    let out = Command::new("git")
...
Edit `crates/graph-nexus-cli/src/commands/diff/mod.rs`:
...
```bash
git add crates/graph-nexus-cli/src/commands/diff/git_guard.rs \
        crates/graph-nexus-cli/src/commands/diff/mod.rs \
        crates/graph-nexus-cli/tests/diff_baseline_resolve_test.rs
...
**Files:**
- Implement: `crates/graph-nexus-cli/src/commands/diff/bindings.rs`
- Modify: `crates/graph-nexus-cli/src/commands/diff/mod.rs` (call into bindings)
- Test: `crates/graph-nexus-cli/tests/diff_bindings_test.rs` (create)

- [ ] **Step 1: Write the failing test**

Create `crates/graph-nexus-cli/tests/diff_bindings_test.rs`:
...
Run: `cargo test -p graph-nexus --test diff_bindings_test`
Expected: fails because `run()` still returns "section diff not yet implemented".

- [ ] **Step 3: Implement `bindings.rs`**

Create `crates/graph-nexus-cli/src/commands/diff/bindings.rs`:
...
pub fn run(args: DiffArgs) -> Result<(), GnxError> {
    ...
    if want_bindings {
        let baseline_jsonl = std::env::temp_dir().join(format!("gnx-diff-bindings-{baseline_sha}.jsonl"));
        let current_jsonl  = std::env::temp_dir().join(format!("gnx-diff-bindings-current-{}.jsonl", std::process::id()));
...
```bash
git add crates/graph-nexus-cli/src/commands/diff/bindings.rs \
        crates/graph-nexus-cli/src/commands/diff/mod.rs \
        crates/graph-nexus-cli/tests/diff_bindings_test.rs
...
**Files:**
- Create: `crates/graph-nexus-cli/src/commands/diff/routes.rs`
- Modify: `crates/graph-nexus-cli/src/commands/diff/mod.rs`
- Test: `crates/graph-nexus-cli/tests/diff_routes_test.rs`
...
Run: `cargo test -p graph-nexus --test diff_routes_test`
...
pub fn extract(graph_path: &Path) -> Result<Vec<RouteEntry>, GnxError> {
    let bytes = std::fs::read(graph_path)
        .map_err(|e| GnxError::Output(format!("read graph: {e}")))?;
    let graph = ZeroCopyGraph::from_bytes(&bytes)
        .map_err(|e| GnxError::Output(format!("graph load: {e}")))?;
    // Iterate Route-kind nodes. Adapt to actual graph API.
    let mut routes = Vec::new();
    for node in graph.nodes_of_kind(graph_nexus_core::graph::NodeKind::Route) {
...
fn parse_method_path(s: &str) -> (String, String) {
    // gnx stores route names as "GET /api/users" — split on first space.
...
(Adapt `ZeroCopyGraph::nodes_of_kind` and `parse_method_path` to actual gnx Graph API. If `graph.nodes_of_kind` doesn't exist, iterate `graph.nodes` and filter `kind == NodeKind::Route`.)

- [ ] **Step 4: Integrate into `DiffArgs::run`**

In `crates/graph-nexus-cli/src/commands/diff/mod.rs`, add a routes branch after the bindings handling:

```rust
let mut routes_diff: Option<routes::RoutesDiff> = None;
if want_routes {
    let baseline_graph = std::env::temp_dir().join(format!("gnx-diff-routes-baseline-{baseline_sha}.bin"));
...

    {
        ...
        std::fs::copy(&bg, &baseline_graph)
            .map_err(|e| GnxError::Output(format!("copy baseline graph: {e}")))?;
    }
...
```rust
fn resolve_current_graph_bin(repo_dir: &std::path::Path) -> Result<std::path::PathBuf, GnxError> {
    // Reuse existing graph_path::resolve logic from gnx
    // Or call gnx admin index --dump-graph if needed
    let path = repo_dir.join(".gitnexus-rs/graph.bin");
    if !path.exists() {
        return Err(GnxError::Output(format!(
            "graph.bin not found at {}; run `gnx admin index` first",
            path.display()
        )));
    }
    Ok(path)
...
```bash
git add crates/graph-nexus-cli/src/commands/diff/routes.rs \
        crates/graph-nexus-cli/src/commands/diff/mod.rs \
        crates/graph-nexus-cli/tests/diff_routes_test.rs
...
**Files:**
- Create: `crates/graph-nexus-cli/src/commands/diff/contracts.rs`
- Modify: `crates/graph-nexus-cli/src/commands/diff/mod.rs`
- Test: `crates/graph-nexus-cli/tests/diff_contracts_test.rs`
...
Run: `cargo test -p graph-nexus --test diff_contracts_test`
...
//! response shapes) between two graph snapshots. Mirrors `gnx contracts` extraction.
...
pub fn extract(graph_path: &Path) -> Result<Vec<ContractEntry>, GnxError> {
    ...
    // Adapt to actual contract-extraction logic in gnx contracts command.
    // Skeleton iterates Fetches edges and emits the request/response shape per route.
    let mut out = Vec::new();
    for edge in graph.edges() {
        if edge.rel_type != graph_nexus_core::graph::RelType::Fetches { continue; }
...
```bash
git add crates/graph-nexus-cli/src/commands/diff/contracts.rs \
        crates/graph-nexus-cli/src/commands/diff/mod.rs \
        crates/graph-nexus-cli/tests/diff_contracts_test.rs
...
**Files:**
- Create: `crates/graph-nexus-cli/src/commands/diff/output.rs`
- Modify: `crates/graph-nexus-cli/src/commands/diff/mod.rs` (replace inline emit with formatter)
- Test: `crates/graph-nexus-cli/tests/diff_output_test.rs` (create)
...
Run: `cargo test -p graph-nexus --test diff_output_test`
...
(Adapt: if `etoon` crate isn't a dep of graph-nexus-cli yet, check `Cargo.toml`. If not present, add via `cargo add etoon` or fall back to a custom toon emitter that handles JSON-shaped data.)

- [ ] **Step 4: Replace inline emit in `mod.rs`**

In `crates/graph-nexus-cli/src/commands/diff/mod.rs`, replace the inline `if args.format == "json" ...` block with:
...
```bash
git add crates/graph-nexus-cli/src/commands/diff/output.rs \
        crates/graph-nexus-cli/src/commands/diff/mod.rs \
        crates/graph-nexus-cli/tests/diff_output_test.rs
...
**Files:**
- Modify: `crates/graph-nexus-cli/src/commands/diff/mod.rs` (Already covered the `All` expansion via `want_*` flags; this task adds a test to lock it.)
- Test: `crates/graph-nexus-cli/tests/diff_section_all_test.rs`
...
Run: `cargo test -p graph-nexus --test diff_section_all_test`
...
```bash
git add crates/graph-nexus-cli/tests/diff_section_all_test.rs \
        crates/graph-nexus-cli/src/commands/diff/mod.rs
...
**Files:**
- Modify: `~/.claude/skills/gnx/SKILL.md`

- [ ] **Step 1: Locate the Tool selection table**

Open `~/.claude/skills/gnx/SKILL.md`. Find the section labeled "Tool selection" with the table whose first row is `| Goal | Command |`.
...
```markdown
| HTTP consumer → Route shape drift detection | `gnx shape_check --route <path>? --repo .` (no `--route` = all routes) |
| Cross-commit graph diff (bindings / routes / contracts) | `gnx diff --section <bindings\|routes\|contracts\|all> --baseline <ref> --repo .` (ref: branch/tag/SHA/HEAD~N/`PR/<n>`) |
```
...
```bash
git add ~/.claude/skills/gnx/SKILL.md
...
```bash
cd ~/.claude
git add skills/gnx/SKILL.md
git commit -m "docs(gnx skill): add shape_check + diff; admin contains mcp/verify-resolver"
cd -
```
...
**Files:**
- Modify: `crates/graph-nexus-cli/tests/cli_help_surface_test.rs` (extend with admin help snapshot)
...
```bash
git add crates/graph-nexus-cli/tests/cli_help_surface_test.rs
...
gh pr create --title "feat: Tier B CLI surface + gnx diff command" --body "$(cat <<'EOF'
## Summary

Implements Tier B spec (docs/superpowers/specs/2026-05-16-tier-b-surface-and-diff-design.md):

1. **CLI surface refactor**:
   - `mcp` (serve|tools) moved from top-level to `gnx admin mcp`
   - `verify-resolver` moved from top-level to `gnx admin verify-resolver`
   - `shape_check` un-hidden at top-level + `--route <path>` filter arg
   - `admin mcp tools` gains `--format <json|toon|text>`

2. **New `gnx diff` command** — generalized graph-level cross-commit diff:
...
- [x] Manual: `gnx admin mcp tools --format json` works
- [x] Manual: `gnx shape_check --route /api/users` filters
- [x] Manual: `gnx diff --section all --baseline origin/main` runs end-to-end
...
**Spec coverage:**
- §2.1 In scope items all map: Task 1 (mcp move), Task 2 (verify_resolver move), Task 3 (shape_check unhide), Task 4 (shape_check --route), Task 5 (mcp --format), Tasks 6-13 (gnx diff), Task 14 (skill doc), Task 15 (CLI surface tests).
