...
`crates/graph-nexus-core/src/graph.rs:27-39` — `RelType`：
```
...
不做的代價：graph-nexus 對 modern web stack 的依賴圖**結構性低估**，PR review 給出的 risk level 持續比真實值低 → 用戶 lose trust 後反而是更大傷害。
...
`docs/specs/2026-05-14-graph-nexus-parity-multi-branch-design.md` 沒提框架特化。建議：
