...
**Audience:** A Sonnet subagent dispatched to add one new language to `graph-nexus-analyzer`.
...
Every language has the same shape in `crates/graph-nexus-analyzer/src/<lang>/`:
...
- `crates/graph-nexus-analyzer/src/c/parser.rs` — 127 lines, **smallest and cleanest**, use this as your starting template
- `crates/graph-nexus-analyzer/src/rust/parser.rs` — 183 lines, similar simplicity
- **Do NOT** template from `crates/graph-nexus-analyzer/src/typescript/parser.rs` — it has a route-dedup loop only TS needs
...
`graph_nexus_core::graph::NodeKind` variants you'll use:
...
### 4.1 `crates/graph-nexus-analyzer/Cargo.toml`
...
### 4.2 `crates/graph-nexus-analyzer/src/lib.rs`
...
### 4.3 `crates/graph-nexus-cli/src/commands/analyze.rs`
...
### 4.4 `crates/graph-nexus-core/src/analyzer/pipeline.rs` — **DON'T MISS THIS**
...
1. **You must `cargo build -p graph-nexus-analyzer` and `cargo build -p graph-nexus-cli` and both must succeed before declaring done.** Report the last 20 lines of each build's output in your response.
2. **You must run `graph-nexus-cli analyze` on your fixture and confirm `.gitnexus-rs/graph.bin` exists.** Report `ls -la` of the output.
3. **You must run `graph-nexus-cli context --name <some_symbol>` on your fixture and get a non-error response.** Report the JSON output.
