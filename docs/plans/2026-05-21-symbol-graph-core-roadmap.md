...
| 1 | **T1-5** `Node.uid: u64` | **in-flight PR #293** (stacked on #285) | `Node.uid` switched to `u64`; `ecp_core::uid::compute(kind,path,owner,name)` drives all UID creation via xxh3-64; D1 collision recovery emits BlindSpot `kind: "uid-collision"`. GRAPH_FORMAT_VERSION 6→7. 31 reader/write sites updated; 2712 tests pass. Merged into `fix/t1-11-rename-owner-class`, ships with #285 |
