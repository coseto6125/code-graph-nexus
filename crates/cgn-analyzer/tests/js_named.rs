...
"#,
    ...
    assert!(
        names.contains(&"html"),
        "expected html fn, got: {:?}",
        names
    );
    assert!(
        names.contains(&"json"),
        "expected json fn, got: {:?}",
        names
    );
    assert!(
        names.contains(&"default"),
        "expected default fn, got: {:?}",
        names
    );
}
...
"#,
    ...
    assert!(
        paths.contains(&"/blog"),
        "expected /blog route, got: {:?}",
        paths
    );
    assert!(
        paths.contains(&"/forum"),
        "expected /forum route, got: {:?}",
        paths
    );
    assert!(
        paths.contains(&"/post/:article"),
        "expected /post/:article route, got: {:?}",
        paths
    );
}
...
"#,
    ...
    let paths: Vec<&str> = local.routes.iter().map(|r| r.path.as_str()).collect();
    assert!(
        paths.contains(&"/:user/bob/"),
        "expected /:user/bob/ route, got: {:?}",
        paths
    );
    assert!(
        paths.contains(&"/foo/:ms/"),
        "expected /foo/:ms/ route, got: {:?}",
        paths
    );
}
...
"#,
    ...
    assert!(
        local.routes.is_empty(),
        "app.use(fn) without path should not emit route, got: {:?}",
        local
            .routes
            .iter()
            .map(|r| r.path.as_str())
