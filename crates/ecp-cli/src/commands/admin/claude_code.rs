//! `ecp admin install-hook --claude-code` / `uninstall-hook` / `status`.
...
pub fn run_uninstall(args: UninstallHookArgs) -> Result<(), EcpError> {
    if !args.claude_code {
...
fn parse_events(csv: &str) -> Result<Vec<String>, EcpError> {
    let mut out = Vec::new();
    ...
    if out.is_empty() {
        return Err(EcpError::InvalidArgument("--events list is empty".into()));
    }
    Ok(out)
}

pub(crate) fn prompt_events_tui(action: &str) -> Result<Vec<String>, EcpError> {
    use dialoguer::{theme::ColorfulTheme, MultiSelect};
    let chosen = MultiSelect::with_theme(&ColorfulTheme::default())
        .with_prompt(format!("Select Claude Code hook events to {action}"))
        .items(ALL_EVENTS)
        .interact()
        .map_err(|e| EcpError::Output(format!("TUI: {e}")))?;
    Ok(chosen
        .into_iter()
...
fn merge_entry(settings: &mut Value, ev: &str, exe: &str) -> Result<(), EcpError> {
    let camel = event_kebab_to_camel(ev);
    let cmd = format!("\"{exe}\" hook {ev} --claude-code");

    let root = settings.as_object_mut().ok_or_else(|| {
        EcpError::InvalidArgument("settings.json root is not a JSON object".into())
    })?;
    let hooks_obj = root.entry("hooks").or_insert_with(|| json!({}));
    let hooks_map = hooks_obj.as_object_mut().ok_or_else(|| {
        EcpError::InvalidArgument("settings.json `hooks` field is not an object".into())
    })?;
    let arr_val = hooks_map
        .entry(camel.to_string())
        .or_insert_with(|| json!([]));
    let arr = arr_val.as_array_mut().ok_or_else(|| {
        EcpError::InvalidArgument(format!("settings.json `hooks.{camel}` is not an array"))
    })?;

    // Idempotence: drop any existing entry pointing at `ecp hook <ev>`.
    arr.retain(|e| {
    ...
    if matches!(ev, "pre-tool-use") {
        h.insert(
            "statusMessage".into(),
            Value::String("Enriching with ecp graph context...".into()),
        );
...
fn write_atomic(path: &Path, value: &Value) -> Result<(), EcpError> {
    ecp_core::registry::atomic_write_json(path, value)
        .map_err(|e| EcpError::Output(format!("atomic write {}: {e}", path.display())))
}
