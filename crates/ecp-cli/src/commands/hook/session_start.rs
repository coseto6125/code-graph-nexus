...
//! `~/.claude/hooks/ecp/rules.md`), substitute placeholders from
...
pub fn handle(input: &HookInput) -> Result<(), EcpError> {
    ...
    // Orphan-registry sweep: detached background `ecp admin prune --orphans`.
    ...
    // handled by `spawn_bg`'s non-blocking flock on `<home_ecp>/.prune.lock` —
    ...
    if index_dir_opt.is_some() && Path::new(&input.cwd).join(".ecp/auto-watch").exists() {
        if let Ok(exe) = std::env::current_exe() {
            let exe_str = exe.to_string_lossy();
            let _ = ecp_core::daemon::spawn_detached(&[&exe_str, "watch", "--start"]);
        }
    }
    Ok(())
}
...
fn detect_worktree_needing_index(cwd: &Path) -> Option<String> {
    ...
    let base = Path::new(&toplevel)
        ...
        .to_string();
    Some(format!(
        "ecp index missing in this worktree ({base} @ {branch}). Run `ecp admin index` to index it."
    ))
}
...
/// Detached background `ecp admin prune --orphans` under flock at
/// `<home_ecp>/.prune.lock`. Writes `.prune-complete` on success or
...
fn spawn_orphan_prune() {
    ...
    let lock = home_ecp.join(".prune.lock");
    let complete = home_ecp.join(".prune-complete");
    let failed = home_ecp.join(".prune-failed");
    let log = home_ecp.join("last-prune.log");
