...
impl Registry {
    /// Open / lazily create `~/.ecp/registry.json`. On parse failure,
    ...
    pub fn open(home_ecp: &Path) -> std::io::Result<Self> {
        std::fs::create_dir_all(home_ecp)?;
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
            let mut perms = std::fs::metadata(home_ecp)?.permissions();
            perms.set_mode(0o700);
            std::fs::set_permissions(home_ecp, perms)?;
        }
        let path = home_ecp.join("registry.json");
        let in_memory = RegistryFile::read_or_empty(&path)?;
        Ok(Self {
            home_ecp: home_ecp.to_path_buf(),
            in_memory,
        })
    }
    ...
    pub fn upsert_repo(&mut self, entry: RepoAlias) -> std::io::Result<()> {
        RegistryFile::upsert_repo_atomic(&self.home_ecp, entry)?;
        let registry_path = self.home_ecp.join("registry.json");
