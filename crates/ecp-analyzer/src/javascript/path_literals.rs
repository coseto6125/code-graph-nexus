//! JavaScript-side extractor for `RawPathLiteral` entries. Walks `string`
...
//!
//! Runs as a side pass over the already-parsed `tree` in `javascript/parser.rs`.
...
/// Walk the JavaScript tree and emit one `RawPathLiteral` per path-shaped
/// string or static template literal. Template literals with `${}` are skipped.
pub fn extract_javascript_path_literals(root: Node<'_>, source: &[u8]) -> Vec<RawPathLiteral> {
    let mut out = Vec::new();
...
/// Strip surrounding backticks from a JS `template_string` node.
...
///
/// JS function-boundary nodes: `function_declaration`, `method_definition`,
