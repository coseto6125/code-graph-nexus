...
pub fn run(args: PruneArgs) -> Result<(), cgn_core::CgnError> {
    if args.orphans {
...
fn run_orphan_sweep() -> Result<(), cgn_core::CgnError> {
    let home_cgn = cgn_core::registry::resolve_home_cgn();
    run_orphan_sweep_in(&home_cgn)
}

fn run_orphan_sweep_in(home_cgn: &std::path::Path) -> Result<(), cgn_core::CgnError> {
    let audit = cgn_core::registry::AuditLog::open(&home_cgn.join("audit.log")).ok();
...
mod tests {
    ...
    fn orphan_sweep_removes_repo_group_member_and_index_root() {
        ...
        let missing_common = home_cgn.join("missing-common-dir").join(".git");
        ...
        run_orphan_sweep_in(home_cgn).expect("sweep");

        let registry = RegistryFile::read_or_empty(&home_cgn.join("registry.json")).expect("read");
