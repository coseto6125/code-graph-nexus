...
fn admin_sessions_list_shows_pure_reference_state() {
    ...
    cgn_core::registry::CommitBuildMeta::write_atomic(&commit_dir.join("meta.json"), &cm).unwrap();
    ...
    cgn_core::session::SessionMeta::write_atomic(&sd.join("session_meta.json"), &sm).unwrap();
