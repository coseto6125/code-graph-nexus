use cgn_cli::commands::group::extractors::{registry, ExtractorKind};
...
fn extractor_kinds_distinct() {
    ...
    let http_count = entries
        .iter()
        .filter(|e| e.kind == ExtractorKind::Http)
        .count();
    let grpc_count = entries
        .iter()
        .filter(|e| e.kind == ExtractorKind::Grpc)
        .count();
