#!/usr/bin/env python3
"""Per-language per-symbol parity diff: ecp vs reference gitnexus.
...
ecp registers each lang sub-dir separately (cd into it to query), while
...
# defects:
#   1. cwd-based rs queries hit a per-lang partial index (ecp treated
...
LANG_EXTS: dict[str, list[str]] = {
    ...
    "Swift": [".swift"],
    "C": [".c"],
    # `.h` belongs to Cpp here — both ecp and ref-gitnexus route `.h` through
    # the C++ parser (ecp `Language::from_normalized_path`, ref-gitnexus
...
def is_anon(name: str) -> bool:
    ...
    # Python `_, x = ...`, Swift `for _ in 0..<n`, etc.). ecp filters
